...
import { SearchByLEIs, SearchByTickers, Sidebar, checkSession } from "../Aws/Aws-api";
...
const SearchByLEI = ({ setCountLoading, activeTab }) => {
  const [display, setDisplay] = useState(0);
  const [did, setDid] = useState("");
  const [stateResult, setStateResult] = useState([]);
  const [query, setQuery] = useState("7YNCQQNDK8FBM9BBTK25");
  ...
  const fetchRecords = async (isMounted, CancelToken) => {
    try {
      ...
      const result = await SearchByLEIs(
        query,
        filteredStates,
        orderBy,
        isAsc,
  ...
  return (
    <div className="container">
      <CompanyFilter
        ...
        query={query}
        activeTab={activeTab}
        nameSearch={"bq_organization_lei"}
      />
      <div className="main-content">
        ...
        <div className="search-box search-box-note">Search by: LEI</div>
        <Tags
          multiFilter={multiFilter}
