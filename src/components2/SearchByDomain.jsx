...
import { DomainFilters } from "../constants/DummyConstants";
import { SearchByDomains, Sidebar, checkSession } from "../Aws/Aws-api.js";
...
const SearchByDomain = ({ setCountLoading, activeTab }) => {
  const [display, setDisplay] = useState(0);
  const [did, setDid] = useState("");
  const [query, setQuery] = useState("1010data.com");
  ...
  const fetchRecords = async (isMounted, CancelToken) => {
    try {
      ...
      const result = await SearchByDomains(
        query,
        filteredStates,
        orderBy,
        isAsc,
  ...
  const fetchcounts = async (isMounted, CancelToken) => {
    try {
      ...
      let field = "bq_organization_website";
      const filteredStates = Object.fromEntries(
        Object.entries(multiFilter).filter(([key, value]) => value.length > 0)
      );
  ...
  return (
    <div className="container">
      <CompanyFilter
        ...
        query={query}
        activeTab={activeTab}
        nameSearch={"bq_organization_website"}
      />
      <div className="main-content">
        ...
        <div className="search-box search-box-note">Search by: Domain</div>
          ...
          <div className="topbar">
            <div className="left">
              <span className="portfolio"
                ...
                style={{ border: portfolioItem?.length > 0 ? "2px solid #00bef3" : "2px solid transparent" }}
